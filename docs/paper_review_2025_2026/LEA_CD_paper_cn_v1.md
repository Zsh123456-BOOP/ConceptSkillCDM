...
图 5 给出两点观察。第一，正确率通道是主要贡献来源：NIPS34、EdNet、ASSIST17 与 XES3G5M 上置零该通道分别损失 0.0032、0.0022、0.0017 与 0.0014 的 AUC，使用量与贡献同向。第二，通道的使用模式跟随数据结构：Junyi 上模型把正确率与残差通道的权重学习为零，只保留传播通道——该数据集每名学生对每个概念至多作答一次，剔除自身后同概念统计恒为空，跨概念传播成为唯一可用的输入。残差与传播通道的单独贡献较小但基本非负，与通道权重非负的构造一致。
