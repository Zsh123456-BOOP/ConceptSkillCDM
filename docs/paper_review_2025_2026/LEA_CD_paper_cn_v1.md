...
本节介绍 LEA-CD。框架的两个模块分别落实第 3 节的两条设计要求：LEA 模块以剔除自身的构造把作答统计送入概念能力，GEC 模块把 1 跳邻居的观测传播到目标概念；两者在同一个 Q 掩码能力读出处汇合，经单一 2PL 项输出预测。整体框架如图 4 所示。
...
如观察二与命题 1 所示，作答统计只有在当前样本被剔除后才是可信的输入。LEA 模块据此在训练时为每条样本构造剔除自身的统计，并按观测次数门控其进入能力标量的幅度。
...
两个模块的贡献呈现互补的分布。LEA 的贡献在 NIPS34 与 XES3G5M 上最大（降幅 0.0050 与 0.0041），在 ASSIST17 上也达到 0.0023，这些数据集有足量的同概念作答可供统计；GEC 的贡献集中在 MOOCRadar 与 EdNet（0.0037 与 0.0028），其概念数多或作答分散，跨概念传播的作用更大。ASSIST17 上移除 GEC 后 AUC 仅下降 0.0001，与其概念图规模最小一致。Junyi 上移除 LEA 与 GEC 的降幅分别为 0.0015 与 0.0012，低于两个模块在主要贡献数据集上的幅度。总体而言，LEA 与 GEC 从互补的方向服务同一个能力标量：前者在同概念观测存在处注入剔除自身的统计，后者把 1 跳邻居的观测送到统计为空的概念上。
...
本文对认知诊断中的作答统计特征进行了系统的实证分析。分析发现，这类特征的表面预测能力大部分来自标签泄漏——当前样本的标签以随观测次数减少而放大的偏差混入自身输入；剔除当前样本后统计仍保留可用信息，且相邻概念的统计携带目标概念的信息、集中在图上 1 跳范围内。基于这些发现，我们提出 LEA-CD：LEA 模块以剔除自身的构造与计数门控把统计送入概念能力，GEC 模块沿课程组织形成的概念图把 1 跳邻居的观测传播到稀缺的目标概念，全部信号经单一 2PL 项输出。六个数据集上的实验表明，LEA-CD 在五个数据集上超过最强基线，增益集中在同概念作答较少的子集，通道行为与各数据集的作答结构一致。据我们所知，这是对认知诊断中作答统计特征泄漏机制的首次系统分析；这些发现为在诊断模型中安全使用作答统计给出了可复用的构造方式，也为把该构造嵌入其他诊断骨干、以及在概念层级结构上的传播留下了空间。
