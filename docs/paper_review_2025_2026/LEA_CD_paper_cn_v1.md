...
认知诊断根据学生的历史作答记录与 Q 矩阵估计其对知识概念的掌握水平，为习题推荐与自适应练习提供基础信息。现有方法通常以学生嵌入刻画学生状态，当目标概念在该学生的训练历史中只有很少的同概念作答时，嵌入缺乏可依据的观测；而直接把同概念正确率等作答统计作为输入特征，会把当前样本的标签带入其自身预测，使评测指标被高估。为此，本文提出 LEA-CD：其一，构造逐行扣除当前标签的留一作答统计，经计数门控以非负权重叠加到概念能力上；其二，从训练日志构造不使用标签的概念关系图，将相邻概念的作答统计传播到同概念作答稀缺的目标概念。在六个真实数据集上，LEA-CD 在五个数据集上取得高于最强基线的 AUC，分桶分析显示增益集中在同概念作答少于 6 次的测试子集，与目标场景一致。
...
从教育测量的角度看，对目标概念的掌握估计应当与该概念相关的观测作答建立联系。然而，真实作答日志中这一联系经常是稀缺的：如图 1(a) 所示，在六个公开数据集上，大量测试作答的目标概念在该学生的训练历史中只有很少的同概念作答。Junyi 上全部测试作答的同概念历史不超过一次，MOOCRadar 与 XES3G5M 上分别有 95.4% 与 98.4% 的测试作答少于 6 次。本文将这一场景称为同概念作答稀缺：目标概念缺少足够的同概念历史作答，模型需要依赖间接观测完成诊断。现有图方法通过嵌入平滑缓解稀疏问题，但嵌入无法区分观测充足与观测稀缺两种情形。一个直接的补充方案是把学生在目标概念上的历史正确率等统计量作为输入特征；然而图 1(b) 显示，这类统计一旦把当前样本自身的标签计入，其排序能力会被严重高估，在 Junyi 上一个零参数统计即可达到 AUC 1.0000。因此，作答统计是有效的输入，但其构造方式必须保证当前样本的标签不进入自身预测。
...
基于上述观察，本文提出 LEA-CD（Leakage-free Evidence Anchoring for Cognitive Diagnosis）。针对泄漏风险，防泄漏统计锚定模块（Leakage-free Evidence Anchoring，LEA）在训练时从每条统计中扣除当前样本的标签贡献，并通过计数门控与非负权重把留一统计叠加到概念能力上；针对同概念作答稀缺，概念图校准模块（Graph Evidence Calibration，GEC）从训练日志构造不使用标签的概念关系图，把相邻概念的作答统计沿行随机关系传播到目标概念。两个模块共享同一个标量 2PL 读出，模型没有额外的预测支路。

本文的主要贡献概括如下：

- 本文刻画了认知诊断中的同概念作答稀缺场景，并给出作答统计特征的标签泄漏偏差的显式形式：偏差项为 $(y-\hat m)/(n+2)$，在单次作答占主导的数据集上足以使零参数统计取得完美排序。
...
本节量化两个现象——同概念作答稀缺的普遍性与作答统计的泄漏偏差——并据此给出 LEA-CD 的两条设计要求。分析使用与实验一致的六个数据集与划分。

**同概念作答稀缺的普遍性。** 对每条测试作答，统计其目标概念在该学生训练历史中的同概念作答次数 $n$，按 $n\in[0,1)$、$[1,3)$、$[3,6)$、$[6,12)$、$[12,\infty)$ 分桶。图 1(a) 显示各数据集的分桶占比差异很大：NIPS34 与 EdNet 的多数作答落在高次数分桶，Junyi 的全部测试作答落在最低分桶（每名学生对每个概念至多作答一次），MOOCRadar 与 XES3G5M 分别有 95.4% 与 98.4% 的作答少于 6 次。同概念作答稀缺是普遍现象，模型需要同时处理观测充足与观测稀缺两种情形。

**作答统计的泄漏偏差。** 考虑最简单的统计特征：学生 $s$ 在概念 $c$ 上的拉普拉斯平滑正确率 $(S_{s,c}+1)/(n_{s,c}+2)$，其中 $S_{s,c}$ 为答对次数。若当前样本 $i$（标签 $y_i$）被计入统计，则该统计作为样本 $i$ 自身预测的输入时包含项

$$\Delta_i=\frac{y_i-\hat m_i}{n_{s,c}+2},$$

其中 $\hat m_i$ 为扣除样本 $i$ 后的平滑正确率。$\Delta_i$ 与标签同号，且随 $n_{s,c}$ 减小而增大；当 $n_{s,c}=0$ 时统计完全由当前标签决定，零参数统计即可对该子集取得完美排序。图 1(b) 验证了这一推断：在单次作答占主导的 Junyi 上，语料泄漏构造的 AUC 恰为 1.0000，MOOCRadar 与 XES3G5M 的统计也分别被抬高约 0.16 与 0.33；改用逐行扣除的留一构造后，各数据集的统计回到真实水平（如 Junyi 0.7891、XES3G5M 0.6472）。泄漏造成的高估幅度与低次数分桶的占比同向：同概念作答越稀缺的数据集，未作扣除的统计越不可靠。

**留一构造的性质。** 将样本 $i$ 的标签、计数与残差贡献从其全部输入统计中扣除后，样本 $i$ 的输入与 $y_i$ 无关（翻转 $y_i$ 不改变其输入特征），训练目标无法通过复制标签下降；我们在全部六个数据集上按位验证了该不变性。同时，统计量以非负权重进入能力标量时，模型预测对同概念正确率单调不减，掌握度的解释方向与教育测量约定一致。

上述分析给出两条设计要求：(R1) 作答统计必须以逐行扣除的留一形式进入模型，并保持非负、门控的进入方式；(R2) 低作答次数分桶的诊断需要把相邻概念的观测经概念结构传播过来。第 4 节的 LEA 与 GEC 模块分别落实这两条要求。
...
其中难度 $b_e$ 为标量、区分度 $a_e$ 经 softplus 保持为正。模型没有其他预测支路：作答统计只能通过能力标量影响预测，不存在绕过 $\theta$ 的捷径。训练目标为标准二元交叉熵，另对关系图施加轻量熵正则以避免退化。
