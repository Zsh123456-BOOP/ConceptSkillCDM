...
认知诊断根据学生的历史作答记录与 Q 矩阵估计其对知识概念的掌握水平，为习题推荐与自适应练习提供基础信息。现有方法通常以学生嵌入刻画学生状态；当目标概念在该学生的历史中缺少同概念作答时，嵌入缺乏可依据的观测，把历史正确率等作答统计直接作为输入特征便成为一种常见的补充。值得注意的是，这类特征的预测能力来源一直未被检验：它有多少来自学生的真实状态，有多少来自构造方式本身。本文对此开展实证分析，发现其表面预测能力主要来自标签泄漏——当前样本的标签会混入它自己的统计特征，作答次数越少影响越大，在单次作答占主导的数据集上，一个不经任何训练的统计量即可取得完美排序；而把当前样本从统计中剔除后，这类特征回到真实水平且仍保留可用信息。基于这些发现，本文提出防泄漏统计锚定认知诊断框架 LEA-CD。该框架首先在训练时从每条作答统计中剔除当前样本，得到与自身标签无关的统计特征；随后为每个统计通道配置随作答次数变化的计数门，把门控后的统计量以非负权重叠加到对应概念的能力上；同时从训练日志构造不使用标签的概念关系图，把相邻概念的统计沿关系图传播到缺少观测的目标概念；全部信号经由单一 2PL 项输出预测。在六个真实数据集上的实验表明，LEA-CD 在五个数据集上取得高于最强基线的 AUC，且增益集中在同概念作答较少的测试子集。
...
从教育测量的角度看，对目标概念的掌握估计应当与该概念相关的观测作答建立联系。然而真实日志中这一联系经常是稀缺的：如图 1(a) 所示，在六个公开数据集上，大量测试作答的目标概念在该学生的训练历史中只出现过很少几次，部分数据集的绝大多数测试作答都处于这种状态。本文将该场景称为同概念作答稀缺：目标概念缺少足够的同概念历史作答，模型需要依赖间接观测完成诊断。知识结构理论表明，概念在课程组织中彼此关联、掌握状态沿概念结构相关（Chiu, Douglas, and Li 2009），这提示间接观测可以来自相关概念的作答。

现有方法难以妥善处理这一场景。图方法通过嵌入平滑缓解稀疏，但嵌入无法区分观测充足与观测稀缺两种情形。把学生的历史正确率等作答统计直接作为输入特征是更直接的补充，这类特征在知识追踪中也被广泛使用；但它的预测能力有多少来自学生的真实状态、有多少来自构造方式本身，此前没有被分析过。

为回答这一问题，本文对作答统计特征开展实证分析（第 3 节）。我们先构造了一个不经任何训练、仅由历史正确率算出的统计量，直接用它在测试集上预测，发现只要当前样本被计入它自己的统计，这个统计量的 AUC 就被大幅抬高，在 Junyi 上甚至恰好达到 1.0000（图 1(b)）——表面预测能力大部分来自当前样本的标签混入了自身输入，而非学生的真实状态。接着我们推导了这一泄漏的显式形式，发现其幅度随作答次数减少而增大，观测越稀缺的数据集被抬得越高。把当前样本从统计中剔除后，该统计量回到真实水平，但仍明显高于随机。这最终使我们得出结论：作答统计包含嵌入之外的可用信息，值得进入模型，但必须剔除当前样本，并配合跨概念结构覆盖观测稀缺的区间。
...
- 本文首次分析了作答统计特征在认知诊断中的标签泄漏机制：给出该偏差的显式形式，说明其随观测次数减少而放大、足以支配评测指标，并在六个数据集上验证了该偏差的实际幅度。
- 本文提出 LEA-CD，通过剔除当前样本的统计构造使作答特征与自身标签无关，通过概念图在低作答次数区间补足观测，两者以计数门控在同一能力标量上融合。
...
**作答统计特征与泄漏风险。** 把历史正确率等统计量作为输入特征在知识追踪中较为常见，认知诊断中也有方法引入类似信号。若统计构造把当前样本计入，训练目标可以直接复制标签，验证与测试指标随之被高估；ISGCN 等工作讨论了图上错误信息的传播，但未处理统计特征把自身标签计入的问题（Shao et al. 2025）。本文给出该偏差的显式形式，并通过剔除当前样本的统计构造消除它。

## 3 同概念作答稀缺与标签泄漏的实证分析

如引言所述，作答统计特征在直觉上是同概念作答稀缺场景的自然补充，但其预测能力的来源尚不清楚：现有工作把这类特征当作普通输入使用，没有分析其中有多少来自学生的真实状态、有多少来自构造方式本身。本节在与实验一致的六个数据集与划分上分析这一问题。

**分析协议。** 我们选取不含任何可学习参数的最简统计作为探针：学生 $s$ 在概念 $c$ 上的拉普拉斯平滑正确率 $(S_{s,c}+1)/(n_{s,c}+2)$，其中 $n_{s,c}$、$S_{s,c}$ 分别为训练集中的同概念作答次数与答对次数；对多概念习题取 Q 矩阵内各概念统计的均值，直接以该统计作为预测分数，在测试集上计算 AUC。统计按三种方式构造，三者输入相同、不经任何训练，差别只在当前样本是否参与自身的统计：全量计入构造把包括当前样本在内的全部作答计入（对应把统计特征在全量数据上预计算的常见做法）；计入自身构造只把当前样本计入它自己的统计；剔除自身构造（即留一法，leave-one-out）把当前样本从它的每条统计中剔除。同时，对每条测试作答记录其目标概念的同概念训练作答次数 $n$，按 $n\in[0,1)$、$[1,3)$、$[3,6)$、$[6,12)$、$[12,\infty)$ 分桶。

**观察一：同概念作答稀缺普遍存在。** 图 1(a) 给出各数据集测试作答的分桶占比。NIPS34 与 EdNet 的多数作答有较充足的同概念历史；Junyi 的全部测试作答落在最低分桶（每名学生对每个概念至多作答一次）；MOOCRadar 与 XES3G5M 分别有 95.4% 与 98.4% 的作答少于 6 次，这说明稀缺不是个别数据集的特例，而是模型必须同时面对的两种常态情形——观测充足与观测稀缺。

**观察二：泄漏支配了统计特征的表面预测能力。** 图 1(b) 给出三种构造的测试 AUC。全量计入与计入自身构造在所有数据集上都明显高于剔除自身构造：Junyi 上恰为 1.0000，MOOCRadar 与 XES3G5M 分别被抬高约 0.16 与 0.33，这表明只要当前样本参与自身统计，评测指标就会被严重高估。剔除自身构造回到真实水平（Junyi 0.7891、XES3G5M 0.6472），但仍明显高于随机，这说明扣除泄漏后统计特征依然保留可用信息。同时，高估幅度与观察一中的低次数占比同向，这进一步表明观测越稀缺，泄漏的影响越大。命题 1 解释了这一现象。

**命题 1（泄漏偏差）。** 若样本 $i$（标签 $y_i$）被计入其自身输入的统计，则平滑正确率包含项

$$\Delta_i=\frac{y_i-\hat m_i}{n_{s,c}+2},$$

其中 $\hat m_i$ 为剔除样本 $i$ 后的平滑正确率。$\Delta_i$ 与标签同号，幅度随 $n_{s,c}$ 减小而增大；当 $n_{s,c}=0$ 时统计完全由当前标签决定，对该子集可取得完美排序。由观察一，Junyi 的全部测试作答满足 $n=0$，命题 1 因此预测其全量计入构造的 AUC 恰为 1，与图 1(b) 的测量精确一致。

命题 1 的含义不限于解释单个数据集：它断言每一条作答的泄漏量都应落在随 $n$ 衰减的包络 $\pm(n+1)/((n+2)(n+3))$ 之内，且符号由标签决定。为进一步检验，我们从六个数据集抽取（测试作答，目标概念）对，把每对的实测泄漏量——计入自身与剔除自身两种构造的统计之差——画在（$n$，泄漏量）平面上，结果见图 2。答对与答错的样本被完全分成上下两翼，任何一个 $n$ 上两类点都没有重叠，全部落点被理论包络精确覆盖——标签本身在输入空间中显形，这正是泄漏构造能获得虚高排序能力的直接原因。

![图 2　泄漏量的逐样本检验。每点为一个（测试作答，目标概念）对，横轴为该概念的同概念训练作答次数 $n$，纵轴为计入自身与剔除自身两种构造的统计之差；蓝色为答对、橙红色为答错，黑色虚线为命题 1 给出的理论包络。](figures/fig_leakage_fan.png)

**观察三：邻居概念的统计携带目标概念的信息，且随图距离衰减。** 前两个观察说明统计需要防泄漏地进入模型，但观测稀缺区间的诊断还需要另一个前提成立：其他概念的作答必须真的携带目标概念的信息。知识结构理论认为，概念在课程组织中相互关联，学生的掌握状态沿概念结构相关（Chiu, Douglas, and Li 2009）；若此成立，相邻概念的作答统计应当能够预测目标概念上的表现。为检验这一点，我们完全不使用同概念作答，仅以其他概念的剔除自身统计预测测试作答，并按统计来源分三档：图先验的 1 跳邻居、2 跳邻居与随机概念（每档各取 16 个概念聚合）。图 3 显示，1 跳邻居在全部六个数据集上都明显优于概念均值基线（提升 0.005–0.146），也全面高于 2 跳与随机档，这说明教学组织造就的邻居确实携带目标概念的可用信息，且信息集中在 1 跳范围内。2 跳相对随机档的优势在半数数据集上消失，这进一步表明结构信息随图距离迅速衰减。在完全没有同概念作答的测试子集上，1 跳统计仍取得远高于基线的 AUC（MOOCRadar 上 0.917 对 0.735），这正是观测稀缺区间可以依赖的信息来源。

![图 3　仅用邻居概念统计预测的 AUC 相对概念均值基线的提升，按统计来源分档。每条线为一个数据集；1 跳邻居在所有数据集上高于 2 跳与随机档。](figures/fig_neighbor_decay.png)

上述分析最终使我们得出结论：作答统计特征的表面预测能力大部分来自标签泄漏，剔除当前样本后仍保留嵌入之外的可用信息；相邻概念的统计同样携带目标概念的信息，且集中在图上 1 跳范围内。因此，如何让作答统计在剔除自身标签的前提下进入模型，并把 1 跳邻居的观测传播到稀缺的目标概念，成为本文方法设计的核心：第 4 节的 LEA 模块负责前者，GEC 模块负责后者。
...
整体框架如图 4 所示。训练日志提供两类输入：概念共现结构与作答统计。GEC 由共现结构学习行随机关系图，用于平滑概念状态并为统计传播提供权重；LEA 构造剔除当前样本的作答统计，经计数门控修正概念能力。两个模块在 Q 掩码的能力读出处融合，经单一 2PL 项输出预测。

![图 4　LEA-CD 框架。训练日志分别提供不使用标签的图先验（上）与剔除当前样本的作答统计（下）；关系学习输出行随机概念图，GNN 编码概念状态；LEA 以计数门控通道修正概念能力；Q 掩码读出后经单一 2PL 项输出预测。](figures/fig_framework.png)

### 4.2 防泄漏统计锚定模块（LEA）

**剔除自身的统计。** 对学生 $s$ 与概念 $c$，记训练集中同概念作答次数与答对次数为 $n_{s,c}$ 与 $S_{s,c}$。预测训练样本 $i$ 时，两者先剔除样本 $i$ 自身的贡献（记 $n^{\setminus i}$、$S^{\setminus i}$），再计算平滑正确率
...
该构造具有两条性质。其一是标签无关性：剔除自身把样本 $i$ 的标签、计数与残差贡献从其全部输入统计中扣除，因此样本 $i$ 的输入与 $y_i$ 无关——翻转 $y_i$ 不改变其任何输入特征，训练目标无法通过复制标签下降；我们在全部六个数据集上按位验证了该不变性。其二是单调性：统计量经非负权重进入能力标量，模型预测对同概念正确率单调不减，掌握度的解释方向与教育测量约定一致。
...
**状态平滑与统计传播。** 关系图以两种方式参与预测。其一，概念状态经 $\alpha$ 混合的消息传递平滑：$\mathbf H^{(l+1)}=(1-\alpha)\mathbf H^{(l)}+\alpha\,\bar R\,\mathbf H^{(l)}$，读出得到 $\theta^{\mathrm{state}}$。其二，每个关系头把剔除自身的统计从邻域聚合到目标概念，构成 LEA 的传播通道：

$$e^{\mathrm{prop},h}_{s,c}=\sum_{k}R_h(c,k)\,e^{\mathrm{dir}}_{s,k}.$$

当目标概念自身没有同概念作答时，正确率通道的门控值接近零，传播通道仍可依据相邻概念的作答给出修正，从而覆盖观测稀缺的区间。
...
分别移除两个模块：w/o LEA 去掉全部作答统计与门控修正（模型退化为纯图—嵌入路径），w/o GEC 去掉状态消息传递与传播通道（统计只保留同概念直接进入）。六个种子的结果见表 3 与图 5。
...
![图 5　移除各模块后的 AUC 降幅（×10⁻³），误差棒为六种子标准差的合成。蓝色为 w/o LEA，橙色为 w/o GEC。](figures/fig_ablation.png)
...
![图 6　合并增益随同概念训练作答次数的变化（×10⁻³），阴影为 95% 学生层配对 bootstrap 置信区间。](figures/fig_gain_curve.png)

图 6 呈现先升后降的形态：0–1 次分桶的增益接近零（+0.0005，区间含零），1–3、3–6、6–12 与 ≥12 次分桶的增益依次为 +0.0020、+0.0030、+0.0025、+0.0018，四个分桶的置信区间均不含零，峰值出现在 3–6 次分桶。该形态与模块的工作方式一致：目标概念完全没有同概念作答时，计数门的取值接近零，统计修正量也接近零；出现少量作答后，嵌入尚未从稀疏观测中学到该学生—概念组合，统计量的边际价值最大；观测继续增多，嵌入自身已能刻画该组合，增益幅度减小但保持为正。逐数据集看，NIPS34 的 1–3 次分桶增益达 +0.0051（区间 [+0.0016, +0.0087]），XES3G5M 的 1–3 次分桶为 +0.0052（[+0.0030, +0.0074]），低次数子集的收益在这两个数据集上最为明显。

### 5.5 统计通道的行为分析

为区分"模型使用了哪个通道"与"该通道对预测的贡献"，对每个数据集统计各通道的实际使用量（对 $\theta$ 的平均修正幅度），并把该通道权重置零后重新评测验证 AUC。

![图 7　各统计通道的使用量与置零后的 AUC 降幅。颜色区分正确率、残差与两个传播通道；右上角为正确率通道占主导的数据集，Junyi 的正确率与残差通道使用量为零。](figures/fig_channel_scatter.png)

图 7 给出两点观察。第一，正确率通道是主要贡献来源：NIPS34、EdNet、ASSIST17 与 XES3G5M 上置零该通道分别损失 0.0032、0.0022、0.0017 与 0.0014 的 AUC，使用量与贡献同向。第二，通道的使用模式跟随数据结构：Junyi 上模型把正确率与残差通道的权重学习为零，只保留传播通道——该数据集每名学生对每个概念至多作答一次，剔除自身后同概念统计恒为空，跨概念传播成为唯一可用的输入。残差与传播通道的单独贡献较小但基本非负，与通道权重非负的构造一致。

## 6 结论

本文研究了认知诊断中的同概念作答稀缺场景，通过实证分析给出作答统计特征的标签泄漏偏差的显式形式，并据此提出 LEA-CD：剔除当前样本的统计构造使作答特征与自身标签无关，计数门控与非负权重把统计叠加到概念能力上，概念图把相邻概念的观测传播到目标概念，全部信号经单一 2PL 项输出。六个数据集上的实验表明，LEA-CD 在五个数据集上超过最强基线，增益集中于同概念作答较少的子集，通道行为与各数据集的作答结构一致。后续工作将探索把防泄漏的统计锚定作为通用组件嵌入其他诊断骨干，以及在概念层级结构上的统计传播。
