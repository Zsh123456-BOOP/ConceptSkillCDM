...
> 中文论文初稿 v0.1。本文稿用于与师兄/导师讨论论文主线、公式链条与实验组织；后续英文投稿时需要进一步压缩、补 baseline 表和统一图编号。
...
**用途**：放在 Introduction，用于说明目标概念没有被学生历史直接覆盖时，模型需要可审计路线。
...
### Figure 2：数据现象与 CRG retrieval

**用途**：数据现象 + CRG 充分性。使用已有实验图，caption 要避免过度强调“单概念题”，改为强调“直接证据稀疏 + 可桥接概念路线”。

推荐 caption：

> 三个核心数据集展示不同形式的概念证据缺口。Junyi 没有题内概念共现，几乎完全依赖 sequence route；assist_09 和 assist_17 是 item edge 稀疏但 sequence evidence 较强的平衡场景。下方检索实验表明，train-only CRG 在 held-out concept transition retrieval 上显著优于 self-only、random 和 degree-random baseline。

### Figure 3：CRG support dependence and evidence-control gap

**用途**：CRG 预测层 support dependence。必须降调：assist_17 是主证据，assist_09 是 support-dependence，junyi weak。

推荐 caption：

> CRG support corruption 提供数据集依赖的必要性证据。assist_17 中，破坏 evidence-supported CRG route 比同度随机 support 更伤害 AUC 和 BCE，因此是 prediction-level support dependence 的主证据。assist_09 中，evidence corruption 与 degree-random 接近，因此只能说明模型依赖候选 support 空间。Junyi 的 prediction-level corruption 较弱，主要用于 CRG 数据现象和 retrieval evidence。

### Figure 4：LCRF counterfactual

**用途**：LCRF 必要性。统一 y 轴，Junyi 标弱。

推荐 caption：

> no-filter 衡量移除 LCRF 的模块贡献；mean-state 和 shuffle-state 检验真实学生状态是否可被群体平均或错配状态替代。assist_09 和 assist_17 在 mean/shuffle 下大幅下降，说明真实 learner state 不可替代。Junyi 效应较弱，仅作为 CRG 主导数据集报告。

### Figure 5：same-query posterior mechanism case

**用途**：LCRF 机制个案。主图建议使用 assist_17 case。

推荐 caption：

> 同一 query、同一 CRG support 下，不同学生获得不同 posterior route。LCRF 不新增 support concept，只在固定 support 内根据学生状态重排 posterior mass。two-student panel 展示机制个案，而不是全样本统计性证明。
