...
"""Compact module recipes for AE-focused training/ablation.

Training/runtime defaults stay in ``src.config``.  This file records only the
module-level recipe overrides needed by the selected CRG/LCRF configuration.
Use ``get_effective_config`` when a caller really needs the full main.py
...
MAIN_JUNYI_OVERRIDES: Dict[str, Any] = {
    # Junyi uses the dataset default module recipe.
}


MAIN_ASSIST17_OVERRIDES: Dict[str, Any] = {
    # E/personalized relation and AE prediction-head recipe
    "personal_rank": 8,
