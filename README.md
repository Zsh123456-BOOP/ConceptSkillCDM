...
当前 v4 读出针对一个明确瓶颈：共享的一维 `theta` 方向无法充分表达学生与题目的匹配。模型在 Q 聚合后增加一个由题目概念共同生成、零初始化的低秩方向，与共享能力方向共同形成唯一的 `theta(student,item)`；同概念题共享该方向，避免每题自由向量记忆训练交互。它仍然进入同一个 2PL 公式，不是第二条预测残差。
