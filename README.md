...
当前 v5 读出针对一个明确瓶颈：共享的一维 `theta` 方向无法充分表达学生与题目的匹配。模型在 Q 聚合后增加一个由题目概念共同生成、零初始化的低秩方向；同概念题共享该方向，避免每题自由向量记忆训练交互。低秩项只作为有界修正：`theta = theta_base + tanh(raw_q_match) / rank`，rank=8 时最多改变 ±0.125 个能力单位，因此不能接管共享 2PL 主预测。它仍然进入同一个 2PL 公式，不是第二条预测残差。
