...
def build_command(job: JobSpec, generate_diagnosis: bool) -> List[str]:
    config_path = job.save_dir / COMMAND_CONFIG_NAME
    config_payload = dict(job.params)
    config_payload.update(
        {
            ...
            "seed": job.seed,
            "generate_diagnosis": bool(generate_diagnosis),
        }
    )
    config_path.parent.mkdir(parents=True, exist_ok=True)
    with open(config_path, "w", encoding="utf-8") as f:
        json.dump(config_payload, f, indent=2, ensure_ascii=False, sort_keys=True)
